...
def floor_value(root, value):
    # checking if the root is there or not
    if root is None:
        return None

    # checking if the required floor value is the value of the root itself
    if root.data == value:
        return root.data

    # if the root value is greater than the required floor value then searching through the left subtree to find the floor
    if root.data > value:
        return floor_value(root.left, value)

    # if the floor is still not found then search the right subtree
    floorValue = floor_value(root.right, value)
    if floorValue is not None and floorValue <= value:
        return floorValue
...
value = int(input("Enter the floor value to find from the given tree: "))
print("The floor value for",value,"form the given tree is:",floor_value(root, value))